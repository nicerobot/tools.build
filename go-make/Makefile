...
# │ push-tooling in nicerobot/repository-tools (sourced from                    │
# │ nicerobot/build-tools/go-make). It is owned upstream — an in-tree edit is   │
...
# │                                                                            │
# │   • Change shared behavior  -> edit nicerobot/build-tools/go-make, then     │
# │     re-run the push from repository-tools.                                  │
...
# with nicerobot/build-tools/go-tooling/go.mod — see TOOL_PKGS). This Makefile
...
# base (nicerobot/build-tools/runtime), which bakes the unprivileged user, certs
